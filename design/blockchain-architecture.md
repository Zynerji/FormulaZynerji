...
### 4.2 The innovation incentive — M11 (token-free, v0.5.0)
The chain records **who uploaded a given innovation first.** The reward for innovating is **not a payment** — there are no tokens and no championship bounty. It is:

- **(a) the transient head-start (4.1)** — you are fast *first*, and score championship points, before rivals can manufacture a copy; and
- **(b) reputational recognition** via the published **Innovation Index** (a leaderboard of who originated what).
- **Attribution:** on upload, a team **declares provenance** — original, or derived from on-chain element X. A geometric-similarity check + dispute process (every design is public) assigns each innovation to its first appearance.

> Net effect: copying is free and instant (field stays compressed), **and** being first pays off through results + reputation (innovation stays attractive). But the head-start alone doesn't *compel* grid-wide innovation — so the **mandatory minimum-spend floor (§4.4, M12) is the load-bearing innovation-forcing lever**, and the field then **self-balances by copying** (`financial.md` Art. 3). No artificial currency is needed.
...
| Risk | Guard |
|------|-------|
| **Free-riding** (copy-only team lives off the pool, never originates) | **Mandatory minimum-spend floor (M12, §4.4)** compels contribution; the head-start (M11) rewards leading — the floor does the heavy lifting |
...
| Innovation-Index gaming (trivial "innovations" farmed for credit) | Index credit only on genuine cross-team adoption; minimum-significance threshold |
| Off-chain "secret" testing to pre-develop before forced upload | Development is limited by the budget cap (M1); only on-chain parts may *run*, so secrecy buys nothing raceable |
...
- **The minimum-spend floor level (M12)** — the keystone that keeps innovation alive in a token-free copy economy; and the **Innovation Index significance threshold** (recognition only — no tokens, no bounty).
...
| Innovation incentive (head-start + recognition; no tokens) | `regulations/financial.md` Art. 8.4 |
