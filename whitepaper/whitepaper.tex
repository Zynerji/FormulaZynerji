...
\begin{center}\includegraphics[width=0.78\textwidth]{car_hero.png}\\[-2pt]
{\footnotesize\textcolor{ink}{Concept render — 2.5\,L two-stroke uniflow diesel, era-kitbash chassis (Blender Cycles, path-traced).}}\end{center}
...
The mechanics are specified to the record level (\texttt{blockchain-spec.md}). Crucially, scrutineering is a \emph{two-clause} test, because a manufactured part can never be bit-identical to its CAD: (i)~a \textbf{cryptographic} check that the fitted part cites an on-chain design whose canonical hash is unaltered and whose timestamp precedes the upload deadline --- which makes a secret or back-dated part impossible; and (ii)~a \textbf{metrology} check that the physical part conforms to that disclosed design within tolerance. Cheating is thereby reduced from ``hide a secret part'' (impossible) to ``manufacture out of tolerance'' (a conventional, detectable problem).
...
\fig{blockchain_protocol}{0.99}{The spine's mechanics: the race-weekend gate (A), the on-chain \texttt{PartRecord} schema (B), and two-clause scrutineering --- design-hash plus physical-conformance (C).}
...
Boost (derived) & PR $\approx$3.3 ($\approx$2.2\,bar gauge)\\
BMEP / rev ceiling & 26\,bar / 7000\,rpm\\
Peak power & $\approx$758\,kW ($\approx$1015\,hp)\\
Peak torque & $\approx$1155\,N\,m @ 4200\,rpm\\
Power-to-weight & $\approx$1.25\,kW/kg\\
BSFC / piston speed & $\approx$190\,g/kWh / $\approx$20\,m/s\\
